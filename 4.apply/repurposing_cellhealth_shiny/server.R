...
shinyServer(function(input, output) {
  ...
  output$bar_chart <- renderPlot(
    {
      ...
      target <- get_target(rank_df, cell_health_model_select)
  ...
  output$scatter_plot <- renderPlot(
    {
      ...
      if (scatter_plot_type == "Cell Health") {
        scatter_gg <- build_cell_health_scatter(
          moa_df, moa_compound_df, dmso_df, cell_health_model_select, target
        )
      } else {
  ...
  output$rank_plot <- renderPlot({
    ...
    rank_df$to_highlight <- ifelse(
      rank_df$target == cell_health_model_select, "black", "white"
      )
