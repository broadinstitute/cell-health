...
get_correlation_matrix <- function(df1, df2, gene_name){
  df <- df1 %>% 
    filter(Metadata_gene_name == gene_name ) %>%
    select( one_of(c(common_features, "Metadata_cell_line","Metadata_gene_name","Metadata_pert_name"))) %>% 
    mutate(Metadata_experiment = 'CRISPR') %>%
    rbind( df2   %>% 
      filter(Metadata_gene_name == gene_name ) %>%
      select( one_of(c(common_features, "Metadata_cell_line","Metadata_gene_name","Metadata_pert_name"))) %>% 
      mutate(Metadata_experiment = 'LUAD')
      ) %>%
...
for (i_gene_name in common_genes[[1]]) {
  cm <- get_correlation_matrix(Pf.cell_health$data, Pf.luad$data, i_gene_name)
  corrplot(cm,title = i_gene_name, diag = FALSE, mar = c(0,0,1,0))
}

gene_name = 'CCND1'
cm_myc <- get_correlation_matrix(Pf.cell_health$data, Pf.luad$data, gene_name)
corrplot(cm_myc, title = gene_name, diag = FALSE, mar = c(0,0,1,0))
...
corrplot(cor_matrix,title = "collapsed genes", diag = FALSE, mar = c(0,0,1,0))
...
for (i_gene in as.vector(common_genes$Metadata_gene_name)) {
  ...
  corrplot(cm, diag = FALSE, title = i_gene, mar = c(0,0,1,0))
