...
# df <- right_join(Pf.cell_health$data, null_thresholds, by = 'Metadata_cell_line')
# df <- right_join(df, correlations, by = 'Metadata_pert_name')
# 
# strong_genes <- df %>% 
#   select(correlation, null_threshold, Metadata_cell_line, Metadata_pert_name, Metadata_gene_name ) %>% 
#   mutate(good_signal = (correlation > null_threshold)) %>% 
#   filter(good_signal) %>% 
...
for (i_gene in as.vector(common_genes$Metadata_gene_name)) {
  ...
  #corrplot(cm, diag = FALSE, title = gene_name[1], mar = c(0,0,1,0))
